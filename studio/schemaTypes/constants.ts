export const specimenSizes = [
    {title: 'Large (> 12cm)', value: 'large'},
    {title: 'Medium (< 12cm)', value: 'medium'},
    {title: 'Small (< 6cm)', value: 'small'},
    {title: 'Tiny (< 3cm)', value: 'tiny'},
    {title: 'Chip (< 1.5cm)', value: 'chip'},
    {title: 'Tiny Chip (< 0.5cm)', value: 'tinyChip'},
  ];

export const colors = [
    {title: 'Black', value: 'black'},
    {title: 'Blue', value: 'blue'},
    {title: 'Brown', value: 'brown'},
    {title: 'Clear', value: 'clear'},
    {title: 'Colorless', value: 'colorless'},
    {title: 'Copper', value: 'copper'},
    {title: 'Gold', value: 'gold'},
    {title: 'Gray', value: 'gray'},
    {title: 'Green', value: 'green'},
    {title: 'Orange', value: 'orange'},
    {title: 'Pink', value: 'pink'},
    {title: 'Purple', value: 'purple'},
    {title: 'Red', value: 'red'},
    {title: 'Silver', value: 'silver'},
    {title: 'Teal', value: 'teal'},
    {title: 'Violet', value: 'violet'},
    {title: 'White', value: 'white'},
    {title: 'Yellow', value: 'yellow'},
]

export const luster = [
    {title: 'Adamantine', value: 'adamantine'},
    {title: 'Dull / Earthy', value: 'dull-earthy'},
    {title: 'Greasy / Oily', value: 'greasy-oily'},
    {title: 'Iridescent', value: 'iridescent'},
    {title: 'Metallic', value: 'metallic'},
    {title: 'Pearly', value: 'pearly'},
    {title: 'Resinous', value: 'resinous'},
    {title: 'Silky', value: 'silky'},
    {title: 'Subadamantine', value: 'subadamantine'},
    {title: 'Submetallic', value: 'submetallic'},
    {title: 'Vitreous', value: 'vitreous'},
    {title: 'Waxy', value: 'waxy'},
]

export const mohsScale = [1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5, 5.5, 6, 6.5, 7, 7.5, 8, 8.5, 9, 9.5, 10];

export const specimenTypes = [
    {title: 'Mineral', value: 'mineral'},
    {title: 'Rock', value: 'rock'},
    {title: 'Fossil', value: 'fossil'},
    {title: 'Meteorite', value: 'meteorite'},
    {title: 'Gemstone', value: 'gemstone'},
    {title: 'Other', value: 'other'},
]
  